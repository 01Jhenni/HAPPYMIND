...
# Posições das imagens dos jogos
...
# Carregar troféu e som de vitória
...
# Função para desenhar textos na tela com borda
def draw_text(text, font, color, surface, x, y):
    text_obj = font.render(text, True, color)
    text_rect = text_obj.get_rect(center=(x, y))

    # Adicionando a borda
    border_rect = text_rect.inflate(10, 10)  # Aumenta o retângulo para criar a borda
    pygame.draw.rect(surface, BLACK, border_rect)  # Borda preta

    surface.blit(text_obj, text_rect)  # Desenha o texto
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if male_icon_rect.collidepoint(event.pos):
                    ...
                    return  # Volta para o fluxo principal
...
# Função para a tela de seleção de jogo
...
def numbers_game():
    ...
    score = 0
    screen.blit(game1_background, (0, 0))  # Fundo do jogo
    draw_character(character_selected)  # Exibir personagem
...
def colors_game():
    ...
    score = 0
    screen.blit(game2_background, (0, 0))  # Fundo do jogo
    draw_character(character_selected)  # Exibir personagem
    for question, options, correct_answer in questions:
        draw_text(question, font_large, WHITE, screen, screen_width // 2, 100)
        for i, option in enumerate(options):
...
def sounds_game():
    ...
    score = 0
    screen.blit(game3_background, (0, 0))  # Fundo do jogo
    draw_character(character_selected)  # Exibir personagem
    for question, options, correct_answer in questions:
        draw_text(question, font_large, WHITE, screen, screen_width // 2, 100)
        for i, option in enumerate(options):
...
while True:
    if current_screen == "home":
        screen.fill(BLACK)
        screen.blit(home_background, (0, 0))  # Fundo da tela inicial
